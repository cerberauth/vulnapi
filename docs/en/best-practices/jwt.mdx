...
## Never Resolve Keys From the Token Header

The verification key must come from server-side configuration, never from the token itself. An attacker controls every header, so trusting `jwk`, `jku`, `x5c`, or `x5u` lets them sign with a key they generated. If key rotation is required, resolve `kid` against a trusted, server-side key set and — where dynamic retrieval is unavoidable — validate `jku`/`x5u` against a strict allowlist of known URLs.

VulnAPI detects these with the [`jwt.jwk_injection`](../vulnerabilities/broken-authentication/jwt-jwk-injection), [`jwt.jku_injection`](../vulnerabilities/broken-authentication/jwt-jku-injection), [`jwt.x5c_injection`](../vulnerabilities/broken-authentication/jwt-x5c-injection), and [`jwt.x5u_injection`](../vulnerabilities/broken-authentication/jwt-x5u-injection) scans.

## Enforce a Single Expected Algorithm Family

Pin the exact algorithm(s) you expect. Accepting the token's `alg` header enables the [algorithm confusion](../vulnerabilities/broken-authentication/jwt-algorithm-confusion) attack (asymmetric token re-signed as HMAC with the public key) and, for ECDSA verifiers on vulnerable runtimes, the [psychic signature](../vulnerabilities/broken-authentication/jwt-psychic-signature) attack (CVE-2022-21449).

VulnAPI detects these with the [`jwt.hmac_confusion`](../vulnerabilities/broken-authentication/jwt-algorithm-confusion) and [`jwt.psychic_signature`](../vulnerabilities/broken-authentication/jwt-psychic-signature) scans.
...
## Validate Claim Types and Lengths Before Use

Don't assume a claim you expect to be a short string actually is one — a claim decoded straight into application logic (a template, a SQL query, a shell command) without checking its type or length can crash the server, leak a stack trace, or enable injection if it arrives as an unexpected type, an oversized string, or `null`.

VulnAPI detects this with the [`jwt.fuzz`](../vulnerabilities/broken-authentication/jwt-claim-fuzzing) scan, which mutates every claim and flags responses that diverge from the baseline.

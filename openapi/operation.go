...
const (
	...
	otelHeadersCountAttributeKey         = attribute.Key("headers_count")
	otelCookiesCountAttributeKey         = attribute.Key("cookies_count")
...
func (openapi *OpenAPI) Operations(ctx context.Context, client *request.Client, securitySchemes auth.SecuritySchemesMap) (operation.Operations, error) {
	...
	telemetryPathCounter, _ := telemetryMeter.Int64Counter(
		"path.counter",
		metric.WithDescription("Number of paths"),
		metric.WithUnit("{path}"),
	)
	telemetryOperationCounter, _ := telemetryMeter.Int64Counter(
		"operation.counter",
		metric.WithDescription("Number of operations"),
		metric.WithUnit("{operation}"),
	)
	telemetryOperationErrorCounter, _ := telemetryMeter.Int64Counter(
		"operation.error.counter",
		metric.WithDescription("Number of errors in operations"),
		metric.WithUnit("{operation-error}"),
	)
	...
	for docPath, p := range openapi.Doc.Paths.Map() {
		...
		for method, o := range p.Operations() {
			...
			otelAttributes = append(otelAttributes, otelHeadersCountAttributeKey.Int64(int64(len(header))))
			otelAttributes = append(otelAttributes, otelCookiesCountAttributeKey.Int64(int64(len(cookies))))

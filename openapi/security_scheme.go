...
func (openapi *OpenAPI) SecuritySchemeMap(ctx context.Context, values *SecuritySchemeValues) (auth.SecuritySchemesMap, error) {
	...
	telemetrySecuritySchemeCounter, _ := telemetryMeter.Int64Counter(
		"security_scheme.counter",
		metric.WithDescription("Number of security schemes"),
		metric.WithUnit("{security-scheme}"),
	)
	telemetryUnsupportedSecuritySchemeCounter, _ := telemetryMeter.Int64Counter(
		"security_scheme.unsupported.counter",
		metric.WithDescription("Number of unsupported security schemes"),
		metric.WithUnit("{security-scheme}"),
	)
	telemetrySecuritySchemeErrorCounter, _ := telemetryMeter.Int64Counter(
		"security_scheme.error.counter",
		metric.WithDescription("Number of errors in security schemes"),
		metric.WithUnit("{security-scheme-error}"),
	)

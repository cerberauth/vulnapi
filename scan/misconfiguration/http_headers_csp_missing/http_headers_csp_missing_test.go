package httpheaderscspmissing_test
...
func runCheck(op *operation.Operation, store harnessx.ResultStore) (harnessx.Result, error) {
	resource := harnessx.Resource{ID: op.ID, URL: op.URL.String(), Method: op.Method, Data: op}
	return httpheaderscspmissing.Check.RunResource(context.Background(), harnessx.Target{URL: op.URL.String()}, resource, store)
}

func TestHTTPHeadersCSPMissing_Passed(t *testing.T) {
...
func TestHTTPHeadersCSPMissing_Failed(t *testing.T) {
	client := request.GetDefaultClient()
	httpmock.ActivateNonDefault(client.Client)
	defer httpmock.DeactivateAndReset()
	...
	require.True(t, ok)
	assert.Equal(t, httpheadersfetch.CSPHTTPHeader, f.Parameter)
}

func TestHTTPHeadersCSPMissing_PassedWhenCSPSetWithoutFrameAncestors(t *testing.T) {
	client := request.GetDefaultClient()
	httpmock.ActivateNonDefault(client.Client)
	defer httpmock.DeactivateAndReset()

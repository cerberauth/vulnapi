package httpheadersframeoptionsmissing
...
var Def = hxcheckdef.MustParseCheckDefYAML("http_headers_frame_options_missing", checkYAML)
...
var Check = hxcheckdef.NewResourceCheck(Def, func(ctx context.Context, _ harnessx.Target, resource harnessx.Resource, store harnessx.ResultStore) (harnessx.Result, error) {
	...
	if !ok {
		return harnessx.Result{Err: errors.New("http_headers_frame_options_missing: resource missing *operation.Operation")}, nil
	}

	data, ok := fetchResultOf(store, resource.ID)
	if !ok || data.Attempt == nil {
		return harnessx.Result{Err: errors.New("http_headers_frame_options_missing: missing fetch result from dependency")}, nil
	}
	attempt := data.Attempt

	if attempt.Response.GetHeader().Get(httpheadersfetch.XFrameOptionsHTTPHeader) != "" {
		return harnessx.Result{}, nil
	}
	return harnessx.Result{Data: &finding.Finding{
		Parameter: httpheadersfetch.XFrameOptionsHTTPHeader,
		Attempt:   attempt,
	}}, nil
})

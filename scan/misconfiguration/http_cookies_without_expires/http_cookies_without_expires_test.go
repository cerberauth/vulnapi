package httpcookieswithoutexpires_test
...
func runCheck(op *operation.Operation, store harnessx.ResultStore) (harnessx.Result, error) {
	resource := harnessx.Resource{ID: op.ID, URL: op.URL.String(), Method: op.Method, Data: op}
	return httpcookieswithoutexpires.Check.RunResource(context.Background(), harnessx.Target{URL: op.URL.String()}, resource, store)
}

func TestHTTPCookiesWithoutExpires_Skipped_WhenNoCookies(t *testing.T) {
...
func TestHTTPCookiesWithoutExpires_Passed_WhenNoUnsecurePractices(t *testing.T) {
	client := request.GetDefaultClient()
	httpmock.ActivateNonDefault(client.Client)
	defer httpmock.DeactivateAndReset()
...
func TestHTTPCookiesWithoutExpires_Failed_WhenExpiresNotSet(t *testing.T) {
	client := request.GetDefaultClient()
	httpmock.ActivateNonDefault(client.Client)
	defer httpmock.DeactivateAndReset()

package httpcookieswithoutexpires
...
var Def = hxcheckdef.MustParseCheckDefYAML("http_cookies_without_expires", checkYAML)
...
var Check = hxcheckdef.NewResourceCheck(Def, func(ctx context.Context, _ harnessx.Target, resource harnessx.Resource, store harnessx.ResultStore) (harnessx.Result, error) {
	...
	if !ok {
		return harnessx.Result{Err: errors.New("http_cookies_without_expires: resource missing *operation.Operation")}, nil
	}

	data, ok := fetchResultOf(store, resource.ID)
	if !ok || data.Attempt == nil {
		return harnessx.Result{Err: errors.New("http_cookies_without_expires: missing fetch result from dependency")}, nil
	}

	f, skip := httpcookiesfetch.EvalCookies(data.Attempt, func(c *http.Cookie) bool { return !c.Expires.IsZero() })
	if skip {

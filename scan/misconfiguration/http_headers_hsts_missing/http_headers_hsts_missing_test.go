package httpheadershstsmissing_test
...
func runCheck(op *operation.Operation, store harnessx.ResultStore) (harnessx.Result, error) {
	resource := harnessx.Resource{ID: op.ID, URL: op.URL.String(), Method: op.Method, Data: op}
	return httpheadershstsmissing.Check.RunResource(context.Background(), harnessx.Target{URL: op.URL.String()}, resource, store)
}

func TestHTTPHeadersHSTSMissing_Passed(t *testing.T) {
...
func TestHTTPHeadersHSTSMissing_Failed(t *testing.T) {
	client := request.GetDefaultClient()
	httpmock.ActivateNonDefault(client.Client)
	defer httpmock.DeactivateAndReset()
	...
	header.Del(httpheadersfetch.HSTSHTTPHeader)
	...
	require.True(t, ok)
	assert.Equal(t, httpheadersfetch.HSTSHTTPHeader, f.Parameter)
}

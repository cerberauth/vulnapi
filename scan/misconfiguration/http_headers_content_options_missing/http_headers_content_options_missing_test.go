package httpheaderscontentoptionsmissing_test
...
func runCheck(op *operation.Operation, store harnessx.ResultStore) (harnessx.Result, error) {
	resource := harnessx.Resource{ID: op.ID, URL: op.URL.String(), Method: op.Method, Data: op}
	return httpheaderscontentoptionsmissing.Check.RunResource(context.Background(), harnessx.Target{URL: op.URL.String()}, resource, store)
}

func TestHTTPHeadersContentOptionsMissing_Passed(t *testing.T) {
...
func TestHTTPHeadersContentOptionsMissing_Failed(t *testing.T) {
	client := request.GetDefaultClient()
	httpmock.ActivateNonDefault(client.Client)
	defer httpmock.DeactivateAndReset()
	...
	header.Del(httpheadersfetch.XContentTypeOptionsHTTPHeader)
	...
	require.True(t, ok)
	assert.Equal(t, httpheadersfetch.XContentTypeOptionsHTTPHeader, f.Parameter)
}

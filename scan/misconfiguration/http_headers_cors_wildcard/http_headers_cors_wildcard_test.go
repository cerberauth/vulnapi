package httpheaderscorswildcard_test
...
func runCheck(op *operation.Operation, store harnessx.ResultStore) (harnessx.Result, error) {
	resource := harnessx.Resource{ID: op.ID, URL: op.URL.String(), Method: op.Method, Data: op}
	return httpheaderscorswildcard.Check.RunResource(context.Background(), harnessx.Target{URL: op.URL.String()}, resource, store)
}

func TestHTTPHeadersCORSWildcard_Passed(t *testing.T) {
...
func TestHTTPHeadersCORSWildcard_SkippedWhenCORSMissing(t *testing.T) {
	client := request.GetDefaultClient()
	httpmock.ActivateNonDefault(client.Client)
	defer httpmock.DeactivateAndReset()
	...
	header.Del(httpheadersfetch.CORSOriginHTTPHeader)
...
func TestHTTPHeadersCORSWildcard_Failed(t *testing.T) {
	client := request.GetDefaultClient()
	httpmock.ActivateNonDefault(client.Client)
	defer httpmock.DeactivateAndReset()
	...
	op.SetSecuritySchemes([]*auth.SecurityScheme{auth.MustNewAuthorizationBearerSecurityScheme("default", &token)})
	header := getValidHTTPHeaders()
	header.Set(httpheadersfetch.CORSOriginHTTPHeader, "*")
	httpmock.RegisterResponder(op.Method, op.URL.String(), httpmock.NewBytesResponder(http.StatusNoContent, nil).HeaderAdd(header))
	...
	require.True(t, ok)
	assert.Equal(t, httpheadersfetch.CORSOriginHTTPHeader, f.Parameter)
}

package httpcookiesnothttponly_test
...
func runCheck(op *operation.Operation, store harnessx.ResultStore) (harnessx.Result, error) {
	resource := harnessx.Resource{ID: op.ID, URL: op.URL.String(), Method: op.Method, Data: op}
	return httpcookiesnothttponly.Check.RunResource(context.Background(), harnessx.Target{URL: op.URL.String()}, resource, store)
}

func TestHTTPCookiesNotHTTPOnly_Skipped_WhenNoCookies(t *testing.T) {
...
func TestHTTPCookiesNotHTTPOnly_Passed_WhenNoUnsecurePractices(t *testing.T) {
	client := request.GetDefaultClient()
	httpmock.ActivateNonDefault(client.Client)
	defer httpmock.DeactivateAndReset()
...
func TestHTTPCookiesNotHTTPOnly_Failed_WhenNotHttpOnly(t *testing.T) {
	client := request.GetDefaultClient()
	httpmock.ActivateNonDefault(client.Client)
	defer httpmock.DeactivateAndReset()
